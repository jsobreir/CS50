...
class MinesweeperAI():
    ...
    def get_neighbours(self, cell):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                # Ensure the neighbor is within bounds and is not the cell itself
                if 0 <= i < self.width and 0 <= j < self.height and (i, j) != cell:
                    neighbours.add((i, j))  # Add the valid neighbor as a tuple
        return neighbours
    ...
    def add_knowledge(self, cell, count):
        ...
        self.mark_safe(cell)
        # Create a sentence consisting of all the neighbours to the safe cell
        print(self.get_neighbours(cell))
        if self.get_neighbours(cell) is not None:
            for neighbour in self.get_neighbours(cell):
                if neighbour not in self.moves_made and neighbour not in self.mines:
                    new_sentence.cells.add(neighbour)
